# Layered-prefill paper-granularity comparison

## Comparison contract

`layered-prefill` implements the paper's one-group-per-iteration schedule. A
...
| Metric | Legacy | Layered-prefill G2/W64 | Reduction |
| --- | ---: | ---: | ---: |
...
Legacy and layered-prefill use the same graph setting within each table.
Layered-prefill wins all four reported client-side and transfer metrics even
...
returns the first token sooner; layered-prefill targets the paper's looser
...
PYTHONOPTIMIZE=1 python benchmarks/bench_scaled_expert_contention.py \
  --modes legacy layered-prefill-g2-wave64 \
  --repetitions 3 --max-prefill-length 512 \
  --output /tmp/layered_prefill_scaled_paper_fair_r3.json
```
...
PYTHONOPTIMIZE=1 python benchmarks/bench_scaled_expert_contention.py \
  ...
  --repetitions 3 --max-prefill-length 512 \
  --cuda-graph-max-bs 0 \
  --output /tmp/layered_prefill_scaled_paper_fair_graph0_r3.json
```
