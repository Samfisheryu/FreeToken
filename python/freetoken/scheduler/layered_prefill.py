...
class LayeredPrefillExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        self._wave: _LayeredPrefillWave | None = None
    ...
    def begin_wave(self, first_batch: Batch, token_budget: int) -> None:
        ...
        if self._wave is not None:
            raise RuntimeError("a layered prefill wave is already active")
        admission = self._staged_admission
        self._staged_admission = None
        if admission is None or not first_batch.prefill_reqs:
            raise RuntimeError("layered prefill wave has no staged prefill admission")
        ...
        if group_size < 1:
            raise RuntimeError("layered prefill has no resident expert-layer capacity")
        self._wave = _LayeredPrefillWave(
            admission=admission,
    ...
    def prepare_step(self, token_budget: int) -> None:
        ...
        if self._decode_input is not None or self._group_input is not None:
            raise RuntimeError("a layered prefill iteration is already staged")
    ...
    def advance_step(self) -> list[ForwardData]:
        ...
        if group_input is None:
            raise RuntimeError("layered prefill iteration was not prepared")
    ...
    def _begin_iteration_state(
        self,
        wave: _LayeredPrefillWave,
        group_input: ForwardInput,
    ) -> tuple[LayerGroupState, LayerGroupState | None]:
        start_layer = wave.current_layer
        if start_layer == 0:
            if wave.state is not None:
                raise RuntimeError("layered prefill wave was embedded more than once")
        ...
        if wave.state is None or wave.state.next_layer != start_layer:
            raise RuntimeError("layered prefill state is not at the active group")
    ...
    def _try_prefetch_next_group(self, wave: _LayeredPrefillWave) -> bool:
    ...
    def _finish_iteration_decode(
        ...
        if decode_state is None:
            raise RuntimeError("mixed layered prefill group did not produce decode state")
    ...
    def _finish_wave(self, wave: _LayeredPrefillWave) -> list[ForwardData]:
        state = wave.state
        if state is None or state.next_layer != wave.num_layers:
            raise RuntimeError("layered prefill wave completed without final model state")
    ...
    def _log_completed_wave(self, wave: _LayeredPrefillWave) -> None:
        moe_cache = self._engine.moe_offload_cache
        assert moe_cache is not None
        layer_prepares = (
    ...
    def _require_wave(self) -> _LayeredPrefillWave:
        if self._wave is None or self._wave.done:
            raise RuntimeError("no layered prefill wave is active")
        return self._wave


__all__ = ["LayeredPrefillExecutor"]
